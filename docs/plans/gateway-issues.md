---
title: Gateway crate — enhancement backlog (owned by Strategos)
description: Enhancements to sensei-hq/gateway (v0.4.6) that Strategos owns; each filed as a gateway-repo issue (create → implement → close) and released via the lockstep tag bump, sequenced before its dependent Strategos phase.
type: plan
status: backlog
created: 2026-07-23
depends_on:
...
## Strategos-side crate-migration tasks (NOT gateway-repo issues)

These are Strategos code/config fixes to the **v0.4.6** crate reality — do them at the start of the reworked Phase 2a (needs the real API + a compile loop; do not blind-edit in the doc phase):
