...
drop policy if exists devices_read on device.devices;
drop policy if exists devices_access on device.devices;
create policy devices_access on device.devices for select to authenticated
  using (
    tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
    and (profile_id = auth.uid() or core.has_capability('device.manage'))
  );
revoke all on device.devices from anon, authenticated, public;
grant select on device.devices to authenticated;
grant select, insert, update, delete on device.devices to service_role;
