...
(function () {
  ...
  const { Switch, ProviderDot, Pill, Tag } = window.StrategosUI;
  const { FALLBACK_CHAIN, ROUTERS, modelById, money } = window.StrategosData;
          ...
          <div className="flex items-center" style={{ gap: 18, flexWrap: 'wrap', marginBottom: 18 }}>
            <div style={{ flex: 1, minWidth: 220 }}>
              <div className="flex justify-between" style={{ marginBottom: 8 }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--ink-mute)' }}>Budget remaining</span>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 600, color: tone(100 - remaining) }}>{remaining}%</span>
              </div>
              ...
              return (
                <div key={step.model}>
                  <div className="flex items-center" style={{ gap: 12, padding: '12px 14px', borderRadius: 'var(--radius-lg)', opacity: skipped ? 0.5 : 1, transition: 'all var(--dur) var(--ease)',
                    ...
                    <span className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-faint)', width: 14 }}>{i + 1}</span>
                    <ProviderDot provider={sm.provider} size={9} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="flex items-center" style={{ gap: 8 }}>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>{step.model}</span>
                        <Tag>{step.role}</Tag>
                        {on && <span className="pill accent" style={{ height: 20 }}><span className="dot" style={{ background: 'var(--accent)' }} />serving now</span>}
                        {skipped && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-faint)' }}>· skipped</span>}
                      </div>
                      <div className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 3 }}>{step.rule} · {sm.route}</div>
                    </div>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12.5, fontWeight: 600, color: step.price === 0 ? 'var(--success)' : 'var(--ink-mute)' }}>{step.price === 0 ? 'free' : money(step.price) + '/M'}</span>
                  </div>
        ...
        <div style={{ padding: '8px 0' }}>
          {ROWS.map((r) => (
            <div key={r[0]} className="flex items-center justify-between" style={{ padding: '11px var(--space-5)', borderBottom: '1px solid var(--paper-edge)' }}>
              ...
              <span className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--ink)' }}>{r[1]}</span>
            </div>
          ))}
        </div>
            ...
            {ROUTERS.map((r) => {
              ...
              return (
                <div key={r.id} className="flex items-center gap-3">
                  ...
                  <span style={{ flex: 1, fontFamily: 'var(--font-mono)', fontSize: 12.5, color: 'var(--ink)' }}>{r.id}</span>
    ...
    return (
      <div className="view-pad wide rise">
        <div className="page-hd">
          <div>
            <div className="zs-eyebrow">Routing</div>
            <h1 className="zs-h1" style={{ marginTop: 4 }}>Spend it like it's yours</h1>
            <p className="zs-body" style={{ marginTop: 6, maxWidth: 640 }}>Fallbacks keep work flowing when money or providers run out. Spend <em>limits</em> live in Organization; this is how the gateway behaves against them.</p>
          </div>
          <Pill icon="org">budget limits → Organization</Pill>
        </div>
