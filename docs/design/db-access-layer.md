...
| Target table | E(r) | E(w) | UI pages | Churn / remap |
|---|---|---|---|---|
| ~~catalog.model_overrides~~ / ~~tenant_model_state~~ **(DROPPED — RATIFIED 2026-08-05)** | — (enablement now derived) | — (managed via chains) | S: models, overview; T: home, ask, compare, models | **Superseded by `chains_for_tenant`.** Enablement = chain membership (a model is usable iff in a tenant's active, key-configured chain). The 4 old readers (ledger.rs:377/411 `/v1/models`+available, chat.rs:264 direct-`model=` gate, config_loader.rs:162 snapshot) + writer (rpc.rs:1144-1189 `/rpc/models/set-enabled`) all derive from the view. Per-tenant pricing = deferred router-level modifier, not per-model. |
...
1. ✅ **DONE `d974097` — Fix `has_capability()` → `effective_role_permissions`** (RW2 live-bug); still TODO in this phase: apply FORCE RLS, tighten device/spaces read policies, remove MV grants. *Security-correctness, no shape change.* (Verify `service_role` has `bypassrls` before FORCE RLS — dev connects as superuser so it's unaffected; confirm prod service_role.)
2. ✅ **DONE `8a0d024`+`a4d1f8a` — fold the trailing-ALTER DDLs inline** (inference_calls, budget_nodes, documents +3, document_versions +4, document_assets +5, document_embeddings +9 incl. generated tsv) so declared==live. *Pure DDL hygiene.*
3. **Create the 8 domain schemas** (empty; register in `design.yaml`). *Zero-blast.*
4. ⚠️ **Lookup tables** (permissions, capability_types, licenses, features, classifications) — seed-managed, additive, low-blast → OK in Phase 0.

> **CORRECTION (2026-08-04, evidence-based — enum conversion is NOT zero-blast; it MOVES into the per-table move phases below).** The original Phase 0 said "convert varchar+CHECK → enum, no surface reads these directly." **Proven false by executing one slice (`execution_location`):** the C1 hot path *binds* these columns — `store.rs` does `.bind("local")` (a Rust `&str`) into the target column, and Postgres **rejects a text param into an enum column** (`ERROR: column is of type … but expression is of type text; you will need to rewrite or cast`). Verified empirically with a parameterized insert; the live gateway's `/v1/chat` insert would break. So each column's `varchar+CHECK → enum` conversion has **runtime Rust blast-radius** (the `.bind()` needs a `sqlx::Type`-derived enum or a `$N::<schema>.<enum>` SQL cast) AND view/MV/RLS dependencies (e.g. `execution_location` feeds the `analytics_model_mix_daily` MV; `plane` feeds 2 views; `classification` feeds spaces RLS). **Therefore: create each enum type + convert its column(s) TOGETHER with that table's move phase (1–7 below), in the same commit that rewrites `store.rs`/reads for the schema-qualified names + FKs** — not as an isolated Phase-0 pass. The `execution_location` slice was reverted (dev restored to varchar, gateway insert re-verified OK) pending its home phase (Phase 6, the ledger overhaul, where store.rs is rewritten anyway).
...
### Phase 3 — Enablement + pricing via `chains_for_tenant` (RATIFIED 2026-08-05 — supersedes the model_overrides fold)
9. **Drop `catalog.model_overrides` + `public.tenant_model_state`.** Model enablement is NOT a stored per-tenant toggle — it is **derived from chain membership**: a model is usable for a tenant iff it appears in one of that tenant's active, **key-configured** chains. Consolidate `effective_chain_models` (tenant-override resolution) + `viable_chain_models` (drop routers with no active credential) into ONE shield **`chains_for_tenant`** that also (a) **keyless-safe guard:** excludes a step only when its `router.requires_key` AND the tenant has no active `keyvault.router_credentials` — a keyless local router (ollama) with no credential stays viable; and (b) **projects pricing** (`model_endpoints.cost_per_*` list price now; per-tenant tier is a *deferred* router-level discount/rate modifier — the view is the `list × modifier` seam, ledger snapshots the effective cost so history never recomputes).
   - **Slice A:** build `chains_for_tenant`; repoint the 4 read/gate sites to derive from it — `/v1/models` (admin: full catalog + `available` derived), `/v1/models/available` (distinct in-chain models), the direct-`model=` gate (chat.rs:264 → chain membership), and config_loader's catalog snapshot (config.rs:162).
   - **Slice B:** retire `tenant_model_state` + `model_overrides` + their `rework.sql` grants/policies + DDL; rewrite/remove `/rpc/models/set-enabled` (enablement managed by editing chains); drop `effective_chain_models`/`viable_chain_models` if fully superseded.
   - Encrypted keys stay in the deny-all `keyvault` (Phase 2); non-secret per-tenant router pricing/expiry config sits beside them. `provider_overrides` gets the same derive-from-chains review later.
