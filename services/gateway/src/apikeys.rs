...
/// Mint `sk_str_<env>_<prefix>.<secret>` — 48-bit lookup prefix + 256-bit secret,
...
pub fn mint() -> Result<NewApiKey, argon2::password_hash::Error> {
    ...
    let env = std::env::var("STRATEGOS_ENV").unwrap_or_else(|_| "dev".to_string());
    let full = format!("sk_str_{env}_{prefix}.{secret}");
    ...
    Ok(NewApiKey { prefix, hashed_secret, full })
}
...
mod tests {
    ...
    fn mint_parses_and_verifies_roundtrip() {
        let k = mint().unwrap();
        assert!(k.full.starts_with("sk_str_"));
    ...
    fn parse_rejects_malformed() {
        ...
        assert!(parse("sk_str_dev_prefixonly").is_none()); // no secret
        assert!(parse("sk_str_dev_.secret").is_none()); // empty prefix
    }
}
