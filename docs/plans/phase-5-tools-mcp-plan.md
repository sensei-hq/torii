...
- **Test scenarios:**
  - Given a fresh DB, When `dbd import` runs, Then `config.mcp_servers` contains the seeded `torii` + `filesystem` `stdio` demo servers as platform-scoped rows.
