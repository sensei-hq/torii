...
(function () {
  ...
  const { ProviderDot, ExecBadge, Meter, useWorkspace, WorkspaceChip, PageHeader } = window.StrategosUI;
  ...
  function cascadePath(node, trail) {
    const t = [...(trail || []), node];
    if (node.kind === 'user') return t;
    for (const c of (node.children || [])) { const r = cascadePath(c, t); if (r) return r; }
    return null;
  }
  ...
  function execRow(r) { return { local: r.route === 'Ollama', mixed: r.route === 'mixed' }; }
  ...
  function RequestsView({ scope = 'org' }) {
    ...
    const excRows = HISTORY.filter((r) => r.status !== 'ok');
    const listRows = mine ? HISTORY.filter((r) => r.user === 'm.okafor') : excRows;
    const [sel, setSel] = useState(0);
    const cur = listRows[Math.min(sel, listRows.length - 1)];
    const PATH = cascadePath(BUDGET_TREE) || [];
            ...
            <div className="grid-split">
              <div className="card" style={{ overflow: 'hidden' }}>
                <div className="card-hd">
                  ...
                  <span className="pill"><span className="dot" style={{ background: 'var(--success)' }} /> this morning</span>
                </div>
                <div style={{ overflowX: 'auto' }}>
                  <table className="tbl tbl-stack" style={{ '--tbl-min': '560px' }}>
                            ...
                            <td data-th="Route">
                              ...
                              <div className="flex items-center gap-2" style={{ marginTop: 3 }}>
                                ...
                                {r.route === 'mixed'
                                  ...
                                  : <ExecBadge local={r.route === 'Ollama'} region={GATEWAY_REGION} />}
                              </div>
                            </td>
            ...
            <div className="grid-split">
              <div>
                <div className="card" style={{ overflow: 'hidden' }}>
                  ...
                  <div className="card-hd" style={{ borderTop: '1px solid var(--paper-edge)' }}><span className="zs-eyebrow">Instances worth a look</span><span className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-mute)' }}>select to trace</span></div>
                  <div>
                    {listRows.map((r, i) => {
                      const st = STATUS[r.status];
                      return (
