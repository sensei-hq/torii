    ...
    return (
      <div className="view-pad wide rise">
        <div className="page-hd">
          <div>
            <div className="zs-eyebrow">Onboarding</div>
            <h1 className="zs-h1" style={{ marginTop: 4 }}>Tenant registration</h1>
            <p className="zs-body" style={{ marginTop: 6, maxWidth: 640 }}>Stand up a governed gateway for your organization. Each step provisions one part of the tenant — identity, residency, routing, budgets and people.</p>
          </div>
          <Pill kind={pct === 100 ? 'success' : 'accent'} icon={pct === 100 ? 'check' : 'info'}>{done} of {steps.length} complete</Pill>
        </div>
        ...
        <div className="card" style={{ padding: 'var(--space-5)', marginBottom: 'var(--space-5)' }}>
          <div className="flex items-center justify-between" style={{ marginBottom: 10 }}>
            ...
            <span className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 500, color: 'var(--accent)' }}>{pct}%</span>
          </div>
            ...
            <div style={{ padding: '4px 0' }}>
              {FIELDS.map(([k, v], i) => (
                <div key={k} className="flex items-center justify-between gap-3" style={{ padding: '11px var(--space-5)', borderBottom: i < FIELDS.length - 1 ? '1px solid var(--paper-edge)' : 'none' }}>
                  ...
                  <span className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--ink)', textAlign: 'right' }}>{v}</span>
                </div>
              ))}
            </div>
