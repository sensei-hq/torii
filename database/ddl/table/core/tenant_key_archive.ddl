-- database/ddl/table/core/tenant_key_archive.ddl
...
, encrypted_dek bytea       not null
    -- A superseded DEK, still sealed under the master KEK. Layout as core.tenant_keys.
...
'Superseded per-tenant DEKs, retained after core.tenant_keys rotation (V4).
- Populated by the vault''s rotate_dek: the prior DEK is copied here before the new one
  replaces it in core.tenant_keys.
- rotate_dek re-seals every active router_credentials row under the new DEK in the SAME
  ...
  core.tenant_keys on KEK rotation (rotate_kek).
