...
(function () {
  ...
  const { ExecBadge, OfflineBanner, useEnv, ProviderDot, RoutingPanel, usePinnedModel, SaveSnippetButton, useWorkspace } = window.StrategosUI;
  const { GATEWAY_REGION, modelById } = window.StrategosData;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
        ...
        <div className="flex items-center gap-3" style={{ padding: '0 var(--space-6) var(--space-4)', flexWrap: 'wrap' }}>
          <button className="flex items-center gap-2" onClick={() => window.StrategosWorkspace.openSwitcher()} title="Switch workspace  ⌘K"
            ...
            <span style={{ font: '600 13px var(--font-ui)', color: 'var(--ink)' }}>{space}</span>
              ...
              <div style={{ maxWidth: 820, margin: '0 auto' }}>
                ...
                {task === 'draft' && (
                  <div className="flex items-center gap-3" style={{ marginTop: 'var(--space-4)', padding: 'var(--space-3) var(--space-4)', borderRadius: 'var(--radius)', background: 'var(--accent-soft)', border: '1px solid oklch(0.58 0.15 35 / 0.25)' }}>
                    ...
                    <span style={{ flex: 1, fontSize: 'var(--text-sm)', color: 'var(--ink)' }}>Draft saved to <b>{space}</b> · <span className="clf clf-confidential" style={{ verticalAlign: 'middle' }}><span className="d" />Confidential</span></span>
