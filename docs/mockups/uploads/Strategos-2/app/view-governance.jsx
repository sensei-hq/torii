...
(function () {
  ...
  const AUDIT = [
    ...
    { t: '08:30', who: 'j.lee',  cat: 'access', text: <span>signed in via Okta · SAML</span> },
  ...
  const FEATURES0 = [
    { name: 'Playground (RAG)', def: 'on', lock: false },
    { name: 'Model pinning', def: 'on', lock: false },
    { name: 'External MCP tools', def: 'off', lock: true },
    { name: 'Web fetch tool', def: 'off', lock: true },
    { name: 'Data export · CSV', def: 'on', lock: false },
    { name: 'Agents & plans (beta)', def: 'off', lock: true },
  ];
  const DEVICES = [
      ...
      <div className="view-pad wide rise">
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div className="card-hd">
            ...
            <span className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-mute)' }}>{total.toLocaleString()} items</span>
          </div>
        ...
        <div className="card" style={{ overflow: 'hidden', marginTop: 'var(--space-5)' }}>
          <div className="card-hd">
            ...
            <span className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-mute)' }}>{features.filter((f) => f.lock).length} locked · {features.filter((f) => !f.lock).length} overridable</span>
          </div>
          <div style={{ padding: '4px 0' }}>
            {features.map((f, i) => (
              <div key={f.name} className="flex items-center gap-3" style={{ padding: '10px var(--space-5)', borderBottom: i < features.length - 1 ? '1px solid var(--paper-edge)' : 'none' }}>
                <Icon name={f.def === 'on' ? 'check' : 'lock'} size={15} tone={f.def === 'on' ? 'success' : 'mute'} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--ink)' }}>{f.name}</div>
                  <div className="mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-mute)' }}>default {f.def} · {f.lock ? 'locked for all roles' : 'members can override'}</div>
                </div>
                <span className="tag">{f.lock ? 'locked' : 'overridable'}</span>
                <Switch on={f.lock} onClick={() => setFeatures((fs) => fs.map((x, j) => (j === i ? { ...x, lock: !x.lock } : x)))} label={'Lock ' + f.name} />
              </div>
            ))}
          </div>
          <div className="card-foot dashed"><Icon name="lock" size={14} tone="mute" /><span><b>Locked</b> features can’t be changed by a space or member; <b>overridable</b> ones inherit unless a space or user opts out.</span></div>
        </div>
