...
create or replace function metering.rollup_apply(
  p_tenant  uuid,
  p_call_id uuid
) returns void
language plpgsql
...
as $$
declare
  v_call        metering.inference_calls%rowtype;
  v_first       boolean;
  v_day         date;
...
begin
  ...
  if v_first then
    ...
    do update set
       ...
       latency_ms_sum         = usage_daily.latency_ms_sum  + coalesce(v_call.duration_ms, 0),
       latency_ms_count       = usage_daily.latency_ms_count + 1,
       local_only_calls       = usage_daily.local_only_calls + v_local_only,
       savings_unpriced_calls = usage_daily.savings_unpriced_calls + v_unpriced;
  end if;
...
revoke execute on function metering.rollup_apply(uuid, uuid) from public;
grant  execute on function metering.rollup_apply(uuid, uuid) to service_role;

comment on function metering.rollup_apply is
