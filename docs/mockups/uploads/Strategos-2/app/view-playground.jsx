...
(function () {
  ...
  function PlaygroundView() {
    ...
    React.useEffect(() => {
      ...
      if (h && h.preset) { const p = h.preset; if (p.level != null) setLevel(p.level); setT((s) => ({ ...s, guardrails: !!p.guardrails, citations: !!p.citations, rerank: !!p.rerank, autotune: !!p.autotune, retention: !!p.retention })); }
    }, []);
    const m = modelById(model);
    const ans = ANSWERS[level];
    const runsLocal = m.route === 'Ollama' || !meta.online;
                    ...
                    {inspect && (
                      <div className="rise" style={{ marginTop: 8, border: '1px solid var(--paper-edge)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
                        {level === 3 ? (
                          ...
                          <div>
                            {RETRIEVED.slice(0, level === 0 ? 2 : 3).map((r) => (
                            ...
                            <div className="mono" style={{ padding: '6px 10px', fontSize: 'var(--text-xs)', color: 'var(--ink-faint)' }}>top {level === 0 ? 2 : 3} of {level === 0 ? '1,240 chunks' : '24 candidates'} · scored against your query</div>
                          </div>
                        )}
                      </div>
                    )}
