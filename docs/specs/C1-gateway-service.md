...
- **Secrets & credential custody (DECISIONS §2 W4).** Provider credentials (`router_credentials`) are decrypted **only** inside C1 at call time via the F3 DEK/KEK envelope; the plaintext key/token never enters a response, log, trace, or error. F3 must land before C1 handles any real credential; production KEK lives in a cloud KMS/HSM (`TORII_KEK` is local-dev only). Startup logs *counts* of resolved credentials, never values.
